...
func GetHostInfo(ctx context.Context, tr it.Transport, req *CLIRequest) (*HostInfoResponse, error) {
	...
	if req.Token == "" {
		err = errors.Join(err, fmt.Errorf("you must supply a vault token for the /v1/sys/host-info endpoint"))
	}

	if err == nil {
		stdout, stderr, err1 := tr.Run(ctx, command.New(
			fmt.Sprintf("%s read -format=json sys/host-info", req.BinPath),
	...
	if err != nil {
		return nil, errors.Join(fmt.Errorf("get vault host info: vault read sys/host-info"), err)
	}
