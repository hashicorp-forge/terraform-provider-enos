...
output "ami_ids" {
  ...
  value = {
    ubuntu = { for arch in local.architecture_filters : (arch == "x86_64" ? "amd64" : arch) => data.aws_ami.ubuntu[arch].id }
    rhel   = { for arch in local.architecture_filters : (arch == "x86_64" ? "amd64" : arch) => data.aws_ami.rhel[arch].id }
  }
}
