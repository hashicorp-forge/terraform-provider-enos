...
var (
	distDir      = flag.String("dist", "", "the output directory of goreleaser that build the artifacts")
	bucketPath   = flag.String("bucket", "", "the S3 bucket path")
	providerName = flag.String("provider-name", "terraform-provider-enos", "the name of the provider")
	providerID   = flag.String("provider-id", "hashicorp.com/qti/enos", "the name of the provider")
	timeout      = flag.Duration("timeout", time.Duration(15*time.Minute), "maximum allowed time to run")
	level        = zap.LevelFlag("log", zap.InfoLevel, "the log level (error, warn, info, debug, trace)")
)
